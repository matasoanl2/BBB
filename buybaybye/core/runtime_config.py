...
class BettingConfig:
    ...
    max_stake_percent_of_bank: float
    strategy_name: str
    bet_delay_min: float
    bet_delay_max: float
    debug_enabled: bool
...
def validate_runtime_config(config: RuntimeConfig) -> None:
    ...
    if config.betting.max_stake_percent_of_bank < 0 or config.betting.max_stake_percent_of_bank > 100:
        raise ValueError("[ERROR] BET_MAX_STAKE_PERCENT_OF_BANK должен быть в диапазоне от 0 до 100.")
